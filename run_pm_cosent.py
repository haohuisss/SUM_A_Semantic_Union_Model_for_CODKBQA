...
datasets = [
    ...
    for f in ['train_no_mask', 'dev_no_mask', 'test_no_mask']
]
...
model = encoder
model.compile(loss=cosent_loss, optimizer=Adam(1e-5),metrics=['accuracy'])
# model.compile(loss=cosent_loss, optimizer=Adam(1e-5))


def compute_corrcoef(x, y):
    """Spearman相关系数
    """
    #a = scipy.stats.pearsonr(x,y).correlation
...
class Evaluator(keras.callbacks.Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        if val_score > self.best_val_score:
            ...
            model.save_weights('./Output/SIM/%s.cosent_pm__no_mask_1e-5_30_1.weights' % task_name)
        print(
