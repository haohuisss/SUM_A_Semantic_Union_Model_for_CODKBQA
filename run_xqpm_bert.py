...
class SimProcessor(DataProcessor):
    def get_train_examples(self, data_dir):
        file_path = os.path.join(data_dir, 'train_data_no_mask.txt')
    ...
    def get_dev_examples(self, data_dir):
        file_path = os.path.join(data_dir, 'dev_data_no_mask.txt')
    ...
    def get_test_examples(self, data_dir):
        file_path = os.path.join(data_dir, 'test_data_no_mask.txt')
...
class BertXQPM:
    ...
    def __init__(self, batch_size=args_xqpm.batch_size):
        self.mode = None
        self.max_seq_length = args_xqpm.max_seq_len
        self.tokenizer = tokenization.FullTokenizer(vocab_file=args_xqpm.vocab_file, do_lower_case=True)
    ...
    def model_fn_builder(self, bert_config, num_labels, init_checkpoint, learning_rate,
        ...
        def model_fn(features, labels, mode, params):  # pylint: disable=unused-argument
            ...
            (total_loss, per_example_loss, logits, probabilities) = BertXQPM.create_model(
    ...
    def get_estimator(self):
        ...
        bert_config = modeling.BertConfig.from_json_file(args_xqpm.config_name)
        label_list = self.processor.get_labels()
        train_examples = self.processor.get_train_examples(args_xqpm.data_dir)
        num_train_steps = int(
            len(train_examples) / self.batch_size * args_xqpm.num_train_epochs)
        num_warmup_steps = int(num_train_steps * 0.1)

        if self.mode == tf.estimator.ModeKeys.TRAIN:
            init_checkpoint = args_xqpm.ckpt_name
        else:
            init_checkpoint = args_xqpm.output_dir

        model_fn = self.model_fn_builder(
            bert_config=bert_config,
            num_labels=len(label_list),
            init_checkpoint=init_checkpoint,
            learning_rate=args_xqpm.learning_rate,
        ...
        config.gpu_options.per_process_gpu_memory_fraction = args_xqpm.gpu_memory_fraction
        config.log_device_placement = False

        return Estimator(model_fn=model_fn, config=RunConfig(session_config=config), model_dir=args_xqpm.output_dir,
    ...
    def generate_from_queue(self):
        while True:
            ...
            features = list(self.convert_examples_to_features(predict_examples, self.processor.get_labels(),
                                                              args_xqpm.max_seq_len, self.tokenizer))
            yield {
    ...
    def train(self):
        ...
        if args_xqpm.max_seq_len > bert_config.max_position_embeddings:
            raise ValueError(
                "Cannot use sequence length %d because the BERT model "
                "was only trained up to sequence length %d" %
                (args_xqpm.max_seq_len, bert_config.max_position_embeddings))

        tf.gfile.MakeDirs(args_xqpm.output_dir)

        label_list = self.processor.get_labels()

        train_examples = self.processor.get_train_examples(args_xqpm.data_dir)
        num_train_steps = int(len(train_examples) / args_xqpm.batch_size * args_xqpm.num_train_epochs)

        estimator = self.get_estimator()

        train_file = os.path.join(args_xqpm.output_dir, "train.tf_record")
        self.file_based_convert_examples_to_features(train_examples, label_list, args_xqpm.max_seq_len, self.tokenizer,
                                                     train_file)
        tf.logging.info("***** Running training *****")
        tf.logging.info("  Num examples = %d", len(train_examples))
        tf.logging.info("  Batch size = %d", args_xqpm.batch_size)
        tf.logging.info("  Num steps = %d", num_train_steps)
        train_input_fn = self.file_based_input_fn_builder(input_file=train_file, seq_length=args_xqpm.max_seq_len,
                                                          is_training=True,
                                                          drop_remainder=True)
    ...
    def eval(self):
        if self.mode is None:
            raise ValueError("Please set the 'mode' parameter")
        eval_examples = self.processor.get_dev_examples(args_xqpm.data_dir)
        eval_file = os.path.join(args_xqpm.output_dir, "eval.tf_record")
        label_list = self.processor.get_labels()
        self.file_based_convert_examples_to_features(
            eval_examples, label_list, args_xqpm.max_seq_len, self.tokenizer, eval_file)
        ...
        eval_input_fn = self.file_based_input_fn_builder(
            ...
            seq_length=args_xqpm.max_seq_len,
            is_training=False,
        ...
        output_eval_file = os.path.join(args_xqpm.output_dir, "eval_results.txt")
