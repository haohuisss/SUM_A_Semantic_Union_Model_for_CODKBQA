...
def predict_outline():
    ...
    with graph.as_default():
        ...
        # sb = Sber()
        # evaluator = Evaluator()
        # encoder_xq, model_xq = sb.get_model(config_path, checkpoint_path)
        # model_xq.load_weights('./Output/XQ/%s.sbert_xq_1e-5_30.weights' % sb.task_name_xq)
        # encoder_pm, model_pm = sb.get_model(config_path, checkpoint_path)
        # model_pm.load_weights('./Output/SIM/SIM_Data.sbert_pm_mask_1e-5_30_1.weights')
        # tokenizer_sb = sb.get_tokenizer(dict_path)

        #使用COSENT作为ED和PM的模型
        ct = Csent()
        evaluator = Evaluator()
        encoder_xq, model_xq = ct.get_model(config_path, checkpoint_path)
        model_xq.load_weights('./Output/XQ/XQ_Data.cosent_xq_1e-5_25.weights')
        encoder_pm, model_pm = ct.get_model(config_path, checkpoint_path)
        model_pm.load_weights('./Output/SIM/SIM_Data.cosent_pm_mask_1e-5_30_2.weights')
        ...
        print(dic)
        # f_read2 = open('neo4jdata.pkl', 'rb')
        # neo4j_dict = pickle.load(f_read2)
        # print(neo4j_dict)
        ...
        for row in nlpcc_test_data.index[0:]:
            ...
            for j in range(len(result_ra)):
                data = (sentence_l2,result_ra[j][1],1)
                f_pm_data.append(data)
            test_generator_sb = data_generator(tokenizer_sb, f_pm_data, len(f_pm_data))
            pm_score = evaluator.evaluate(test_generator_sb, encoder_pm, ct.l2_normalize)
            ...
            max_idx = ans_score.index(max(ans_score))
        ...
        print(str(ANS_num))
        # #字典保存
        # f_save = open('neo4jdata.pkl', 'wb')
        # pickle.dump(neo4j_dict, f_save)
        # f_save.close()
        #
        # with open("all_errentity_6.txt", "w",encoding='utf-8') as f:
        #     f.write("\n".join(errentity))
        # NNN = 8465
