...
class Sber:
    ...
    def __init__(self, maxlen=64, batch_size=8, epochs=5, task_name_xqpm='XQPM_Data'):
        # 基本参数
        self.maxlen = maxlen
        self.batch_size = batch_size
        self.epochs = epochs
        self.task_name_xqpm = task_name_xqpm
...
class data_generator(DataGenerator):
    ...
    def __iter__(self, random=False):
        ...
        for is_end, (text1, text2, label) in self.sample(random):
            label = int(
                label > 2.5
            ) if random and sb.task_name_xqpm == 'STS-B' else label
...
if __name__ == '__main__':
    ...
    model.load_weights('./Output/XQPM/%s.sbert_xqpm_no_mask_1e-5_30_1.weights' % sb.task_name_xqpm)
    ...
    really_rel_list = []
    rea_r_flag = 0
    with open('./Data/XQPM_Data/test_data_no_mask.txt', 'r', encoding='utf-8') as fd:
